...
| `lmt visual reconstruct <project> <screen_id> --capture-manifest <json> [--method vpqsp\|charuco] [--intrinsics <json\|auto>] [--intrinsics-crosscheck <json>]` | destructive | Multi-view photos → `measurements/measured.yaml` + `measurements/<screen_id>_cabinet_pose_report.json` (model-constrained BA, zero total station). The capture manifest's own `method` field (`vpqsp` \| `charuco`) selects the detector; **`--method` (default `vpqsp`) is a forward-compat guard** — the manifest is authoritative. **vpqsp**: decode self-encoding VP-QSP markers → each marker's `(screen_id, col, row, local_id)` routes directly to its cabinet + nominal local-mm (`+y`-up, center-origin), feeding the SAME `solve_and_emit` BA as charuco (`gauge_strategy=fix_root_cabinet`, root cabinet = gauge). Observability/Stage-A/Stage-B robust trim are shared. Markers from other screens (different `screen_id_code`) are filtered out. The manifest's `screen_mapping.expected_pattern_hash` (if set) is preflight-checked against the captured pattern; if the mapping omits it, reconstruct proceeds but emits a `pattern_hash_unset` warning (the capture↔pattern binding is unverified) — warnings ride `VisualReconstructResult.warnings`. **Intrinsics**: `--intrinsics <path>` (CLI override) > the manifest's `intrinsics` field; the field is now **optional** (omit it for the self-cal flow). **`--intrinsics auto`** (vpqsp only): inline self-calibrate K + distortion from the SAME captured markers (frame-matched, single detection pass) — the displayed marker wall IS the calibration target, using the screen's pixel pitch as the metric scale (Zhang / `cv2.calibrateCamera`, the shared `solve_sl_intrinsics`). `VisualReconstructResult.intrinsics_source` = `auto_self_calibrated` (vs `file`). Unlike `reconstruct-structured-light`, a **flat wall WITHOUT an anchor is ADMITTED** (with a `no_intrinsics_anchor` warning): the known pixel pitch fixes the focal/scale ambiguity, so a known flat target + angularly-diverse shots is well-posed. The solver's own conditioning gates (≥5° pose-rotation diversity, ≥20% image coverage, focal/principal-point stddev caps) still refuse an ill-posed capture (e.g. all shots fronto-parallel) → `observability_failed`(17). With `--intrinsics-crosscheck <anchor.json>` (independent intrinsics) the anti-absorption cross-check compares the self-cal K to the anchor on focal + fx/fy aspect + radial & tangential distortion; deviation (e.g. anisotropic screen pitch / non-1:1 drive absorbed into K) → `observability_failed`(17) BEFORE any write; a malformed anchor → `invalid_input`. `--intrinsics auto` on a **charuco** manifest → `invalid_input` (no marker-grid target). No intrinsics from either source → `invalid_input`. Errors reuse the shared set: `invalid_input`(3/2), `intrinsics_invalid`(16), `detection_failed`(13), `observability_failed`(17), `ba_diverged`(14). |
