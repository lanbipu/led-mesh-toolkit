...
| `lmt export pose-obj <pose_report> <target> --out-dir <dir> [--root <cabinet_id>] [--ground]` | destructive | 从 `cabinet_pose_report.json` 把每块屏单独导出成世界坐标 OBJ（每屏一个文件，顶点烘进共享世界系，导入 disguise 即回到正确相对位置）；Path B 直出路径，绕过 `reconstruct surface`。几何按 `Neutral` 原样输出（pose report 已是 +Y up / +Z outward = disguise 约定，不套坐标适配器）。`--root <cabinet_id>`：以该屏局部系为世界系（它轴对齐落原点），其余屏保持相对位姿；`--ground`：下边缘贴地（基准屏下沿→Y=0；给了 `--root` 时以基准屏为准，物理更低的非基准屏可能 y<0）。`target` ∈ `{disguise, unreal, neutral}`（校验+记录，当前不改轴/单位）。Result: `{target, cabinet_count, files}` |
