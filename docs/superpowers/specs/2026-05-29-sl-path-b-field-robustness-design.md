...
- **S8 剔除可见 + undecidable 硬停（no silent caps / no silent wrong）**：report / DTO 报告每机位 /
  每箱体 / 全局的剔除数，剔除比例高发 WarningEvent；而**几何 undecidable**（2 视图相干矛盾、凹凸分不开）
  → **硬停 `observability_failed`、绝不写出可能错的 measured.yaml**（warning 非致命、文件照写，不够）。
...
- **decode 结果回传（finding-3 修正：原"加 DTO 字段"没有可靠 IPC 通道）**：现 adapter 是**读写出的
  `CorrespondenceFile`**（不读 decode 的 generic `ResultEvent`）来构 `DecodeStructuredLightResult`，所以：
  - **实际用的 `screen_roi`** 加进 `CorrespondenceFile` schema（Python `ipc.py` + Rust adapter `ipc.rs`，
    可选字段、作 detection provenance，corr.json 自描述；reconstruct 忽略它，加可选字段不破 provenance 校验、
    无需 bump schema_version）。adapter 顺带读出可填 `DecodeStructuredLightResult.screen_roi`（若需 Rust 侧可见）。
  - **debug 图**不进 DTO / IPC：写到**确定性路径** `<out>.debug.png`（caller 可推出），dry-run `would_write` 列它。
  - 即不依赖被忽略的 ResultEvent 传值；ROI 走 corr.json、debug 图走约定路径，审计链端到端可观测。
...
多数表决 → 这是欠定的本质二义，**不能**自动判对。**处理（finding-2 修正：warning 不够，因为 WarningEvent
非致命、reconstruct 照样写 measured.yaml → 会静默 ship 一个错的测量文件）：当裁剪检测到某箱体只剩 2 个有效视图
且两视图根本性矛盾（要把整组顶掉才能降残差）→ 在写任何 measured.yaml / pose_report 之前 HARD STOP，
报 `observability_failed`（消息说明"剔除后该箱体 2 视图相干矛盾、欠定"）。** 缓解靠多机位冗余（现场 SOP
要求被剔除触及的箱体 ≥3 视图）；不在本里程碑做"2 票相持仍判对"的求解。
...
③**同一处再叠 IPPE 两解 + 凹凸消歧（见 Part C）**——RANSAC 出 inlier、IPPE 出两分支，分支上抛到模型系
按 **shape_prior nominal 朝向**消歧（front-facing 实测无效），三件事一次改完，不要拆成两遍。
...
- `test_two_view_coherent_hard_stops`：某箱体仅 2 视图、其一相干错，断言系统**硬停 `observability_failed`
  且不写 measured.yaml / pose_report**（finding-2：不接受"只发 warning 仍把可能错的文件写出去"）。
...
## C.3 修法（init PnP 取两解 + shape_prior 主消歧）

> **对抗式 review + cv2 实测修正（推翻上一版把 front-facing 当主判据）**：实测
> `cv2.solvePnPGeneric(SOLVEPNP_IPPE)` 在 10°–70° 斜视角下，两分支法向 **z 分量完全相同**（都朝相机），
> 只有**横向分量符号相反**（如 tilt=55° → `[±0.82, 0, 0.57]`）。所以"front-facing / 法向 z 符号"
> **根本分不开两分支**，不能当判据。真正区分两者的是**横向 tilt 方向**——必须拿**已知 nominal 朝向**去比。
> 用户最初把 shape_prior 排第一是对的。

把 init 用的 PnP 升级成"取两解 + 消歧"：
1. 在（Stage A RANSAC 的）inlier 上跑 `cv2.solvePnPGeneric(obj,img,K,None,flags=SOLVEPNP_IPPE)`
   → 最多 2 个分支（需 OpenCV ≥4）。
2. 消歧（**在模型 / 世界系里比**，不是相机系局部判——见 C.4 改动位置）：
   - **主：shape_prior nominal 朝向 + 弧拓扑一致性**——把两分支各 compose 进模型系（root 锚定为其 nominal），
     取该箱体世界系法向**整体**（尤其横向 tilt）最接近 nominal 弧朝向的分支；再 enforce **曲率符号一致 +
     邻居法向平滑**（整条弧要么按 nominal 一致凹 / 凸，不许 zig-zag）。flat 时 nominal = 共面朝前，同样取最接近者。
   - **次（仅 well-separated 时）：重投影误差比**——无噪声时错分支误差明显更大（实测比值极大），可直接取小的；
     但斜视角 + 真实噪声下两者误差会接近，此时**不可信**，必须回落 nominal 判据。
   - **front-facing 不作判据**（实测两分支 z 符号恒同、零区分力）；仅留作 sanity：两分支都该朝相机，否则数据异常。
3. 返回消歧后的 (R,t)。**若 nominal 也分不开**（两分支与 nominal 同样接近 + 误差比 ≈ 1，例如孤立近正面箱体、
   或 shape_prior 本身不可信）→ 该箱凹凸**判 undecidable**，走 §3 / B.2 的**硬停**（不静默写出可能翻转的结果），
   **不**只发 warning。

## C.4 与 Part B 的合流（同一处 PnP 升级 + 模型系消歧）

Part B Stage A 要 RANSAC inlier、Part C 要 IPPE 两解 + nominal 消歧——合成**一个**升级后的 PnP 例程：
`solvePnPRansac 取 inlier → 在 inlier 上 solvePnPGeneric(IPPE) 取两分支 → 返回两分支 + inlier_mask`。
**关键：消歧不在 `_solve_pnp` 局部做（相机系拿不到 nominal、且 front-facing 无效），而是把两分支上抛到
模型系 assembly 处**——`estimate_nonroot_cabinet_init` / `solve_and_emit` init 段，那里有 nominal 弧朝向，
按 C.3 第 2 步选分支 + 跨箱体一致性。`_solve_pnp` 的两个调用方（`estimate_nonroot_cabinet_init`、
`_pnp_camera`）统一走它。**实现上交付项 ⑨⑩ 与 Part C 是同一处改动，不要拆成两遍写。**

## C.5 接口契约

- **无新 CLI flag / 无新输入**：shape_prior 已随 project 传入；消歧全在 sidecar init。
- **新 nominal 朝向 helper**（nominal.py 纯函数，从 curved 弧几何算每箱 nominal 法向；sidecar 内部，无对外契约面）。
- **report / DTO 不变、无新错误码**：可判定但低裕度 → `WarningEvent`（no silent caps）；**完全 undecidable
  且无冗余视图 → 硬停 `observability_failed`，绝不写出可能翻转的 measured.yaml**（见 §3）。
- 法向约定对齐 `reconstruct_cabinet_geometry`（用一致性单测守住，否则消歧会确定性反向）。
...
- `test_ippe_branches_share_front_facing_zsign`（回归 Codex finding-1）：构造斜视角，断言 IPPE 两分支
  **z 符号相同**（横向相反）→ 证明 front-facing 无区分力、消歧**必须**靠 nominal；并断言 nominal 判据选对分支。
- `test_seeded_flip_is_corrected`：故意喂翻转初值，断言 nominal 消歧把它纠回（非 front-facing）。
- `test_nominal_orientation_picks_branch`：单箱斜视角，断言按 nominal 横向 tilt 选中正确分支。
- `test_undecidable_convexity_hard_stops`：构造 nominal 也分不开 + 无冗余视图，断言**硬停 observability_failed、
  不写 measured.yaml / pose_report**（而非发 warning 照写）。
...
- **"2 视图 + 1 相干错" / 凹凸 undecidable 的本质二义不自动消解**：见 B.2 根本限度 + C.3——**硬停
  `observability_failed`、不写结果**（不是发 warning 照写），靠多机位冗余（被剔除触及的箱体 ≥3 视图）缓解；
  不在本里程碑做"少数服从多数失效时仍判对"的求解。
...
⑤ `CorrespondenceFile`(Python+Rust)加 `screen_roi` provenance 字段 + adapter 读出；debug 图写确定性
`<out>.debug.png`(不靠被忽略的 ResultEvent)；⑥ `manifest.rs` CLI 串；
...
**Part C 凹凸消歧**（与 ⑨⑩ 同一处 PnP 改动）：⑰ init PnP 取 IPPE 两解、分支上抛模型系按 **shape_prior
nominal 朝向 + 弧拓扑一致性主消歧**（front-facing 实测无效仅 sanity；reproj-err 仅 well-separated 时用）；
undecidable → 硬停 `observability_failed` 不写结果；⑱ nominal.py 新 nominal-朝向纯函数（从 curved 弧几何算）；
⑲ sidecar pytest(S9 + same-z-sign 回归 + 翻转纠正 + undecidable 硬停 + 法向约定一致性) + 重建 binary。
（无 CLI / DTO / 错误码改动。）
