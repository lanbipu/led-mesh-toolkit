...
- **P6 反吸收硬门（no silent wrong，新增·对应 Codex high）**：合成台**注入三类 pitch/1:1 误差**
  ——(a) 各向同性 scale、(b) 各向异性 sx≠sy、(c) 平滑非线性 remap——`--intrinsics auto` 全程，
  断言**每类在导出前都被报出来**（(a) 走 procrustes 阈值；(b)(c) 走独立交叉校验 → `WarningEvent`
  或 `observability_failed`），并**断言它没有被静默吸进 K**（即：开了反吸收护栏 vs 关护栏对照，
  关护栏时低 residual 但 metric 错、开护栏时报错或拒绝）。这是 spec 的 no-ship 红线。
...
- **锚点来源**（取其一）：①现有棋盘格 `visual calibrate`（与屏无关的物理靶，最干净）；②一个
  **focal 先验 + 容差**（厂商标称 / 镜头档案）；③多视角的几何自洽冗余（cross-view consistency，
  屏侧系统误差在不同机位会留下不一致痕迹）。
- **判据**：`auto` 解出的 K（尤其 fx/fy 比、畸变量级）若与独立锚点偏差**超出协方差/容差** →
  这就是"屏侧 pitch/1:1 被吸进 K"的信号 → 发 `WarningEvent`（指明"内参与独立锚点不符，疑似屏侧
  pitch/1:1 误差被吸收"）；**平面墙 + 无任何独立锚点**时，`auto` **拒绝声称精准**（降级/
  `observability_failed`，不静默给可能 metric 错的结果）。
...
精准传 3）+ 视差/斜视角约束（覆盖判据收紧）。复用现有 planner 几何/可见性/打分，仅调门槛。
**这是 L1 高阶畸变 + focal 可观测的前提**（平面墙正面拍解不稳，靠斜视角破）。
