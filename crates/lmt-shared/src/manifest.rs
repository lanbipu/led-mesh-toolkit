...
pub fn build() -> ContractManifest {
    ...
    let operations = vec![
        ...
        op("export.pose_obj", "Write one world-frame OBJ per cabinet from a cabinet_pose_report.json (--root re-bases on a cabinet, --ground sits bottom edge at 0)",
           "lmt export pose-obj <pose_report> <target> --out-dir <dir> [--root <cabinet_id>] [--ground]", Destructive, true, false, false, Some("ExportPoseObjResult"), &[0, 2, 3, 4, 6]),
