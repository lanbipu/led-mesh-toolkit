...
pub fn build() -> ContractManifest {
    ...
    let operations = vec![
        ...
        op("visual.decode_structured_light", "Decode a recorded structured-light capture (video or frame dir) into a provenance-stamped screen<->camera correspondence file. --sentinel-threshold (default 0.85, whole-frame mean/255) detects the full-white sentinel frames; lower it (e.g. 0.4) when the screen does not fill the frame or the background is not black (e.g. disguise visualiser gray stage)",
           "lmt visual decode-structured-light <input> <sl_meta> --out <corr.json> [--sentinel-threshold F]", Destructive, true, false, false, Some("DecodeStructuredLightResult"), &[0, 2, 3, 4, 13, 18]),
