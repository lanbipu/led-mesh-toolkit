...
pub fn build() -> ContractManifest {
    ...
    let operations = vec![
        ...
        op("project.save", "Atomic write <dir>/project.yaml from YAML/JSON (stdin or --input)",
           "lmt project save <abs_path>", Destructive, true, true, false, None, &[0, 2, 4, 6]),
        ...
        op("total_station.import", "Trimble CSV -> measurements/measured.yaml + import_report.json",
           "lmt total-station import <project> <screen_id> <csv>", Destructive, true, false, false, Some("TotalStationImportResult"), &[0, 2, 3, 4]),
        ...
        op("reconstruct.surface", "Run reconstruction, write report.json + reconstruction_runs row",
           "lmt reconstruct surface <project> <screen_id> <measurements_rel>", Destructive, true, false, false, None, &[0, 2, 3, 4, 5]),
        op("reconstruct.list_runs", "List reconstruction_runs for a project",
           "lmt reconstruct list-runs <project>", ReadOnly, false, false, true, Some("ReconstructionRun"), &[0, 2, 5]),
        ...
        op("export.obj", "Write an OBJ for a run (target: disguise|unreal|neutral)",
           "lmt export obj <run_id> <target>", Destructive, true, false, false, None, &[0, 2, 3, 5]),
    ];
