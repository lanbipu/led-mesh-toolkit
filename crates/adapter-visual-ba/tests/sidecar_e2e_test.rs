...
fn mock_path() -> PathBuf { fixture("mock_sidecar.sh") }
...
async fn stderr_is_drained_and_included_in_failure_message() {
    let _guard = ENV_LOCK.lock().unwrap();
    env::set_var("LMT_VBA_SIDECAR_PATH", fixture("mock_sidecar_stderr.sh").to_str().unwrap());
    ...
    assert!(s.contains("stderr-line-"), "expected stderr tail in error: {s}");
}
...
async fn slow_progress_consumer_does_not_block_stdout() {
    ...
    assert_eq!(result.frame_strategy_used, lmt_adapter_visual_ba::FrameStrategy::NominalAnchoring);
