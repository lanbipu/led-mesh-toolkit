...
use lmt_shared::data::recent_projects;
...
fn add_recent(
    ...
    // Path normalization 在 lmt_shared::data::recent_projects::upsert_normalized
...
fn load(mode: Mode, abs_path: &str) -> i32 {
    match lmt_app::projects::load_project_yaml_from_path(Path::new(abs_path)) {
...
fn save(
    ...
    match decision {
        ...
        DestructiveDecision::Execute => {
            match lmt_app::projects::save_project_yaml_to_path(Path::new(abs_path), &cfg) {
                Ok(()) => output::ok(
                    mode,
