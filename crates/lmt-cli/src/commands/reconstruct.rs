...
fn surface(
    ...
    match decision {
        DestructiveDecision::DryRun => {
            ...
            let cfg = match lmt_app::projects::load_project_yaml_from_path(&project) {
                Ok(c) => c,
        ...
        DestructiveDecision::Execute => {
            ...
            match lmt_app::reconstruct::run_reconstruction(
                db,
                ...
                Ok(r) => {
                    ...
                    output::ok(mode, payload, |p| {
                        let _ = writeln!(
                            std::io::stdout(),
                            "run #{} written ({})\n  method={} rms={:.3}mm p95={:.3}mm vertices={}",
                            p["run_id"],
                            p["report_json_path"].as_str().unwrap_or(""),
                            p["method"].as_str().unwrap_or("?"),
                            p["estimated_rms_mm"].as_f64().unwrap_or(f64::NAN),
                            p["estimated_p95_mm"].as_f64().unwrap_or(f64::NAN),
                            p["vertices"],
                        );
                    })
                }
...
fn list_runs(mode: Mode, db_arg: Option<&Path>, project_path: &str, screen_id: Option<&str>) -> i32 {
    ...
    let rows = match db_opt {
        None => Ok(Vec::new()),
        Some(db) => lmt_app::reconstruct::list_runs_for(db, project_path, screen_id),
    };
    match rows {
        Ok(rows) => output::ok(mode, rows, |list| {
            ...
            for r in list {
                let _ = writeln!(
                    std::io::stdout(),
                    "#{:>4}  screen={}  method={}  rms={:.3}mm  vertices={}  ({})",
                    r.id,
                    r.screen_id,
                    r.method,
                    r.estimated_rms_mm,
...
fn get_run_report(mode: Mode, db_arg: Option<&Path>, run_id: i64) -> i32 {
    ...
    match lmt_app::reconstruct::read_run_report(db, run_id) {
