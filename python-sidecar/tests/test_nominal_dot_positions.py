...
def test_flat_offset_dot_matches_sl_local_mm_translation():
    # nominal_dot_positions_world flips sl_local_mm's +y-UP to the center grid's
    # +y-DOWN before the rigid compose (so multi-row walls stay rigid). For a flat
    # single cabinet R_y=I, so world == center + (lx, -ly, 0). Asserting against
    # the RAW sl_local_mm (un-flipped) would re-encode the multi-row sawtooth bug.
    cab = CabinetArray(cols=1, rows=1, cabinet_size_mm=[500.0, 500.0])
    rect = CabinetRect(col=0, row=0, input_rect_px=[0, 0, 540, 540], pixel_pitch_mm=[500.0/540, 500.0/540])
    ...
    local_m_flipped = np.array([local_m[0], -local_m[1], local_m[2]])
    assert np.allclose(world[7], center + local_m_flipped, atol=1e-9)
...
def test_multirow_flat_wall_is_rigid():
    ...
    # monotonic world-y with screen-v across the whole wall (rigid, no sawtooth)
    order = np.argsort([d.v for d in dots], kind="stable")
    ys = W[order, 1]
    assert all(ys[k] <= ys[k + 1] + 1e-9 for k in range(len(ys) - 1))
    # rigid fit to the true flat grid (u,v pixel -> +y-down world) ~ machine zero
    true = np.array([[d.u * pitch / 1000.0, d.v * pitch / 1000.0, 0.0] for d in dots])
...
def test_curved_cabinet_dot_centroid_is_cabinet_center():
    ...
    for c in range(3):
        ...
        # (FIX 2), so the dot-plane normal is that tile's rotated +z. Oracle it
        # against the tile pose itself, NOT _cabinet_normal_model (reconstruct's
        # separate R_y(+alpha) convention, intentionally not touched here).
