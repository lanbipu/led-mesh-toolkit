...
diverse shots is a well-posed Zhang problem. The y-flip in
``nominal_marker_positions_world`` ([x, y, 0] -> [x, -y, 0]) is applied to the **z=0 local
markers**, where it coincides exactly with the proper rotation R_x(180deg)
(R_x(180deg)·[x, y, 0] = [x, -y, 0]; the z-negation is invisible at z=0). Being a proper
rotation — not a reflection — it is absorbed by calibrateCamera's per-view extrinsics, so K
and handedness are unaffected. (A genuine reflection, det=-1, could NOT be absorbed by a
proper-rotation extrinsic, which is why the z=0 detail is load-bearing.)
"""
